//! Types and functions for macOS-specific process resource usage information
use crate::helpers::syscall;
use crate::libproc::bindings::proc_pid_rusage;
use libc::c_void;
...
/// ```
/// use std::io::Write;
/// use mac_process::libproc::pid_rusage::{pidrusage, RUsageInfoV2};
///
/// fn pidrusage_test() {
///     use std::process;
...
/// }
/// ```
pub fn pidrusage<T: PIDRUsage>(pid: i32) -> Result<T, io::Error> {
    let flavor = T::flavor() as i32;
    let mut pidrusage = MaybeUninit::<T>::uninit();
    #[allow(clippy::pedantic)]
    let buffer_ptr = &mut pidrusage as *mut _ as *mut c_void;
    syscall::cvt_nonnegative(unsafe { proc_pid_rusage(pid, flavor, buffer_ptr.cast()) })?;
    Ok(unsafe { pidrusage.assume_init() })
}
